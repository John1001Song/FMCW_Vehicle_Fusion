...
class AttentionPooling(nn.Module):
    def __init__(self, input_dim):
        ...
        self.attention_weights = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(128, 1),
            nn.Softmax(dim=1)  # Normalize across points
        )

    def forward(self, features):
        weights = self.attention_weights(features)  # Shape: (batch_size, num_points, 1)
        return torch.sum(features * weights, dim=1)  # Weighted sum
...
class RadarModel(nn.Module):
    def __init__(self, input_dim=3, dynamic_dim=3, hidden_dim=128):
        ...
        self.point_encoder = nn.Sequential(
            ...
            ResidualBlock(512, hidden_dim)
        )
        self.dynamic_encoder = nn.Sequential(
            ResidualBlock(dynamic_dim, 64),
            ResidualBlock(64, 128),
            ResidualBlock(128, 256),
            ResidualBlock(256, 512),
            ResidualBlock(512, hidden_dim)
...
class RadarDepthEstimationModel(nn.Module):
    ...
    def forward(self, points, dynamics):
        ...
        pooled_features = self.attention_pooling(attended_features)
