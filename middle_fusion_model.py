...
class RadarDataset(Dataset):
    def __init__(self, input_file, label_file, train_indices_file, val_indices_file, test_indices_file=None, split="train"):
        """
        Load radar sensing data and split into train, validation, and test sets.
        Args:
            ...
            test_indices_file (str, optional): Path to test indices .npy file.
            split (str): One of "train", "val", or "test" to select the appropriate subset.
        """
        try:
...
class RadarModel(nn.Module):
    def __init__(self, input_dim=3, dynamic_dim=3, hidden_dim=128):
        super().__init__()
        # For simplicity, using a two-layer encoder for points and dynamics.
        ...
        self.fusion = nn.Linear(256, hidden_dim)
        # Instantiate the self-attention module here so that its parameters are registered.
    ...
    def forward(self, points, dynamics):
        ...
        # Use the pre-instantiated self.attention module.
