...
class RadarDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Normalize points (example: zero mean, unit variance)
        points[:, :3] = (points[:, :3] - points[:, :3].mean(dim=0)) / (points[:, :3].std(dim=0) + 1e-6)
        points[:, 3:6] = (points[:, 3:6] - points[:, 3:6].mean(dim=0)) / (points[:, 3:6].std(dim=0) + 1e-6)

        return {
            'points': points[:, [0, 1, 2]],    # X, Y, Z coordinates [N, 3]
            'dynamics': points[:, [3, 4, 5]], # Velocity, Range, Bearing [N, 3]
            'range': points[:, 4],            # Range [N]
            'bearing': points[:, 5],          # Bearing [N]
            'intensity': points[:, 6],        # Intensity [N]
            'bbox_gt': labels[0, :7],         # (w, h, l, x, y, z, theta) [7]
            'depth_gt': labels[0, 5]          # Z position (depth GT) [1]
        }
...
class RadarModel(nn.Module):
    def __init__(
        ...
        self.point_encoder = nn.Sequential(
            ...
            nn.Linear(256, 512),
            nn.ReLU(),
            nn.Linear(512, hidden_dim)  # Now 512
        )
    ...
    def forward(self, points, dynamics):
        ...
        # Encode points and dynamics
        point_features = self.point_encoder(points.reshape(-1, points.shape[-1]))       # (batch_size*num_points, 512)
        dynamic_features = self.dynamic_encoder(dynamics.reshape(-1, dynamics.shape[-1])) # (batch_size*num_points, 512)

        # Combine and reshape
        combined = torch.cat([point_features, dynamic_features], dim=1).view(batch_size, num_points, -1)  # (batch_size, num_points, 1024)

        # Pass through TransformerBlocks
        x = combined
...
class RadarDepthEstimationModel(nn.Module):
    ...
    def forward(self, points, dynamics):
        ...
        depth_pred = self.depth_subnet(pooled_features)  # => (batch_size, 1)
        bbox_prediction = self.decoder(pooled_features)   # => (batch_size, 7)
        return bbox_prediction, depth_pred
...
# ---------------------------------------
def custom_loss(bbox_pred, bbox_gt, depth_pred, depth_gt, loss_weights):
    """
    Compute combined loss for bounding box regression and depth estimation.
    Args:
        bbox_pred: [B, 7] tensor
        bbox_gt: [B, 7] tensor
        depth_pred: [B, 1] tensor
        depth_gt: [B] tensor
        loss_weights: tuple (w_bbox, w_depth, w_iou)
    Returns:
    ...
    # Depth Estimation Loss: Smooth L1 Loss
    depth_loss = F.smooth_l1_loss(depth_pred.squeeze(), depth_gt)
    
    # IoU Loss
    iou_loss_value = iou_loss(bbox_pred, bbox_gt)

    # Combine losses
    total_loss = w_bbox * bbox_loss + w_depth * depth_loss + w_iou * iou_loss_value
    return total_loss
...
def train_model(
    ...
    best_val_iou = {t: 0.0 for t in IOU_THRESHOLDS}
    epochs_no_improve = 0
    ...
    for epoch in range(epochs):
        ...
        for batch in train_dataloader:
            ...
            # Forward pass
            bbox_pred, depth_pred = model(batch['points'], batch['dynamics'])
            ...
            mae, rmse = compute_depth_metrics(depth_pred.squeeze(), batch['depth_gt'])
            mae_total += mae
            rmse_total += rmse

        avg_train_loss = total_loss / len(train_dataloader)
        avg_train_iou = {key: iou_results[key] / len(train_dataloader) for key in iou_results}
        avg_mae = mae_total / len(train_dataloader)
        avg_rmse = rmse_total / len(train_dataloader)

        # Validation
        val_loss, val_iou, val_mae, val_rmse = validate_model(model, val_dataloader, loss_weights, device)
        ...
        if val_loss < best_val_loss - 1e-4:
            improvement = True
        for key in IOU_THRESHOLDS:
            if val_iou[key] > best_val_iou[key] + 1e-4:
                improvement = True
                break
        ...
        msg = (
            ...
            f"Train IoU: {pformat(avg_train_iou)}, Val IoU: {pformat(val_iou)},\n"
            f"Train MAE: {avg_mae:.4f}, Val MAE: {val_mae:.4f},\n"
            f"Train RMSE: {avg_rmse:.4f}, Val RMSE: {val_rmse:.4f},\n"
...
def validate_model(model, val_dataloader, loss_weights, device):
    ...
    avg_mae = mae_total / len(val_dataloader)
    avg_rmse = rmse_total / len(val_dataloader)
    return avg_loss, avg_iou, avg_mae, avg_rmse
...
def test_model(model, dataloader, log_file, loss_weights, device):
    ...
    msg = (
        ...
        f"Test MAE: {avg_mae:.4f}, Test RMSE: {avg_rmse:.4f}"
    )
    log_print(msg, log_file)
